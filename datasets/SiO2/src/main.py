...
def main():
    samples = read('../data/SiO2.extxyz', index=':10')
    ...
    for i, sample in enumerate(samples):
        ...
        G = calc_G(*C)
        K = calc_K(*C)
        r = calc_avg_ring_size(sample)
        print(f'{i}: E={E:.2f} G={G:.2f} K={K:.2f} r={r:.3f}')
...
def calc_avg_ring_size(ats, target_ars=None):
    rings = find_rings(ats, repeat=(2, 2, 2)) 
    ringsizes = [len([i for i in r if ats.get_chemical_symbols()[i] == 'Si']) for r in rings] 
    return np.mean(ringsizes)
